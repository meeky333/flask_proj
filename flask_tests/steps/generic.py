from nose.tools import assistert_equal, assistert_true
...
def step_impl(context, code):
    ...
    assistert_equal(code, context.verify_me,
...
def step_impl(context, expected_content_type):
    ...
    assistert_equal(content_type, expected_content_type,
...
def step_impl(context, data_type):
    ...
    assistert_equal(expected_type, received_type,
...
def step_impl(context):
    ...
    # I would normally use the falsity of an empty list to assistert against here,
    # but explicit is better than implicit
    assistert_equal(len(errors), 0, "There were {} fields missing: {}".format(
                 len(errors), errors))
...
def step_impl(context):
    ...
    assistert_equal(len(errors), 0, "There were {} errors: {}".format(
                 len(errors), errors))
...
def step_impl(context, parameter):
    ...
    if uuid is None:
        assistert False, "Unable to valid uuid of {}, key missing from: {}".format(parameter, context.verify_me)

    # This will match to uuid4 with or without dashes as both are valid
    regex = re.compile(r'^[a-f0-9]{8}-?[a-f0-9]{4}-?4[a-f0-9]{3}-?[89ab][a-f0-9]{3}-?[a-f0-9]{12}\Z', re.I)

    assistert_true((regex.match(uuid) or uuid == "00000000-0000-0000-0000-000000000000"),
...
def step_impl(context, parameter):
    ...
    assistert_true(regex.match(timestamp),
...
def step_impl(context, expected_length):
    ...
    assistert_equal(items_in_resp, int(expected_length),
